...
TeleopBebopDrone::TeleopBebopDrone()
{
    ...
    nh_.subscribe("/chatter", 1000, &TeleopBebopDrone::chatterCallback, this);

    std::cout << std::endl;
    std::cout << "Variables set to false"<< std::endl;

    joy_sub = nh_.subscribe("/joy", 1,&TeleopBebopDrone::joyCallback, this);
    _toggle_pressed_in_last_msg = _cam_toggle_pressed_in_last_msg = false;

    std::cout << std::endl;
    std::cout << "Subscriber node set" << std::endl;
	
    pub_takeoff       = nh_.advertise<std_msgs::Empty>("/bebop/takeoff",1);
    pub_land          = nh_.advertise<std_msgs::Empty>("/bebop/land",1);
    pub_toggle_state  = nh_.advertise<std_msgs::Empty>("/bebop/reset",1);


    pub_set_ground = nh_.advertise<std_msgs::Bool>("/bebopdrone/remember_here",1);
    pub_estimate_gain = nh_.advertise<std_msgs::Bool>("/bebopdrone/estimate_gain",1);
    pub_write_to_file = nh_.advertise<std_msgs::Bool>("/bebopdrone/write_to_file",1);

    pub_vel           = nh_.advertise<geometry_msgs::Twist>("/bebop/cmd_vel",1);

    srv_cl_cam        = nh_.serviceClient<std_srvs::Empty>("/bebopdrone/togglecam");
    ...
    std::cout << std::endl;
    std::cout << "Test Mode:" << config_test_mode << std::endl;

    if (config_test_mode > 2)
    {
        // Subscribe to the rostopic "/orb_slam2_mono/pose" to get the ORB pose:
        orb_subscriber = nh_.subscribe("/orb_slam2_mono/pose", 1, &TeleopBebopDrone::OrbCallback, this);

        // Publish the rostopic "/bebopdrone/yawData":
        yawData_publisher = nh_.advertise<std_msgs::Float64>("/bebopdrone/yawData",1);

        // Publish the rostopic "/bebopdrone/svData":
        svData_publisher = nh_.advertise<std_msgs::Float64>("/bebopdrone/svData",1);
    }
}
...
void TeleopBebopDrone::chatterCallback(const std_msgs::String::ConstPtr& msg)
{
  exit (EXIT_FAILURE);
}

void TeleopBebopDrone::joyCallback(const sensor_msgs::JoyConstPtr joy_msg)
{
    ...
    {
       // ROS_INFO("Config Mode is lower than 8");
...
//         std::cout << "twist_joy.angular.z = " << twist_joy.angular.z << std::endl;
    ...
    bool take_off = joy_msg->buttons.at(0);
    if (!_is_flying && take_off)
    {
        ROS_INFO("Button A was pressed, Taking off!");
        pub_takeoff.publish(std_msgs::Empty());
        _is_flying = true;
	ROS_INFO("Published A");
    }
    ...
    /////////////////////////////////////////////////////////////////
    //   BUTTON 5: Set the current position as the ground level    //
    /////////////////////////////////////////////////////////////////
	...
	ROS_INFO("Button RB was pressed");
    ...
    ///////////////////////////////////////////////
    //   BUTTON 6: Write to file with button 6   //
    ///////////////////////////////////////////////
    ...
    {
        ROS_INFO("Button BACK was pressed");
    ...
    ///////////////////////////////////////////////////////////////////////
    //   BUTTON 7: Set the current position as the reference position   //
    ...
    {
        ROS_INFO("Button SELECT was pressed");
	...
	ROS_INFO("Published SELECT");
	
    }
	

    ///////////////////////////////////////////////////////////////////////
    //   BUTTON 8: Increase alpha_rd                                    //
    ///////////////////////////////////////////////////////////////////////
    ...
    {
        ROS_INFO("Button XBOX was pressed");
	...
	ROS_INFO("Published XBOX");
	
    }
	

    ///////////////////////////////////////////////////////////////////////
    //   BUTTON 9: Decrease alpha_rd                                    //
    ///////////////////////////////////////////////////////////////////////
    ...
    {
        ROS_INFO("Button LZ was pressed");
...
void TeleopBebopDrone::send_cmd_vel()
{
...
void TeleopBebopDrone::OrbCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    // Update time:
    time_slam = msg->header.stamp.toSec();
    ...
    if (time_slam > time_slam_prev)
    {
        ...
        z_vel_slam_prev = z_vel_slam;
         ...
         // Update xyz:
        x_slam = msg->pose.position.x; // Positive forward
        y_slam = msg->pose.position.y; // Positive left
        z_slam = msg->pose.position.z; // Positive upward



        // Update rpy:
        tf::Quaternion quaternion_slam(msg->pose.orientation.x,
        ...
        yaw_slam = y_temp; // Positive yaw left
        pitch_slam = p_temp; // Positive pitch up
        roll_slam = r_temp; // Positive roll left
...
void TeleopBebopDrone::ResetAutopilot()
{
...
void TeleopBebopDrone::RunAutopilot()
{
    Publish2CSV();
    //PublishRealTimeData();
...
bool TeleopBebopDrone::OscillationDetected()
{
    bool detected = false;
...
bool TeleopBebopDrone::StabilityDetected()
{
    bool detected = false;
...
void TeleopBebopDrone::ReadDynamicReconfigure()
{
    if (config_test_mode == 3 || config_test_mode == 8)
    {
        if (dt_slam < 1)
...
void TeleopBebopDrone::LockSlamStartPosition()
{
...
void TeleopBebopDrone::LockSlamRefPosition()
{
...
void TeleopBebopDrone::SetGroundLevel()
{
...
void TeleopBebopDrone::ComputeTrueScale()
{
	...
	std::cout << "True scale computed:"  << _true_scale_computed << std::endl;
    }
}


void TeleopBebopDrone::ComputeCommandYaw()
{
    // Yaw hold:
...
void TeleopBebopDrone::ComputeNavCommandXYZ()
{
    if (config_test_mode < 8) // Using a joystick simulator
    {
...
void TeleopBebopDrone::ComputeVertCommandZ()
{
...
void TeleopBebopDrone::SetWaypointsHover()
{
    x_wp = x_start;
    y_wp = y_start;
    z_wp = z_start;
...
void TeleopBebopDrone::SetWaypointsLineFlying(const int &direction)
{
    if (!_ref_pos_locked)
    {
...
void TeleopBebopDrone::SetWaypointsCubeFigureFlying()
{
    if (!_ref_pos_locked)
    {
        std::cout << "First set a reference position! (Button 10)" << std::endl;
...
void TeleopBebopDrone::SetWaypointsVerticalFlying()
{
    if (!_ref_pos_locked)
    {
        std::cout << "First set a reference position! (Button 10)" << std::endl;
...
void TeleopBebopDrone::SetWaypointsHorizontalFlying()
{
    if (!_ref_pos_locked)
    {
        std::cout << "First set a reference position! (Button 10)" << std::endl;
...
void TeleopBebopDrone::ComputeDetectionVariable()
{
    stability_case = 0;
    ...
    if (gain == gain_prev)
    {
        ...
        if (dV_window.size() == config_time_window_js*(1+trunc_factor))
        {
		    ...
		    std::cout << "Oscillation detected" << std::endl;
...
void TeleopBebopDrone::AdaptGain()
{
    ...
    if (_gain_increment_enabled && _detect_oscillation && !_detect_stability)
    {
	    ...
	    std::cout << "No oscillation, but we want to detect it --> decrease virtual height" << std::endl;
	...
	std::cout << "No stability, but we want to detect it --> increase virtual height" << std::endl;
        std::cout << "gain decreased to " << gain <<std::endl;
    }

}

void TeleopBebopDrone::ComputeCriticalGain()
{
...
void TeleopBebopDrone::PublishRealTimeData()
{
...
void TeleopBebopDrone::Publish2CSV()
{
    if (_write_to_file && !_writing_complete)
    {
        ...
        outfile.open("/home/sebastian/Desktop/Practicum/Results/Scale_prediction/distance.csv", std::ios_base::app); // append
        for (unsigned i=0; i < gain_final_vec.size(); i++)
        {
